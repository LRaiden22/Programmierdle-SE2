...
Deno.serve(async (req) => {
  ...
  const data = {
    ...
    paradigm: compare(
      selectedLang.data?.[0]?.paradigm,
      correctLang.data?.[0]?.paradigm,
    ),
    platform: compare(
      selectedLang.data?.[0]?.platform,
      correctLang.data?.[0]?.platform,
    ),
    typesystem: compare(
      selectedLang.data?.[0]?.typesystem,
      correctLang.data?.[0]?.typesystem,
    ),
    purpose: compare(
      selectedLang.data?.[0]?.purpose,
      correctLang.data?.[0]?.purpose,
    ),
    easyness: compare(
      selectedLang.data?.[0]?.easyness,
      correctLang.data?.[0]?.easyness,
    ),
    community: compare(
      selectedLang.data?.[0]?.community,
      correctLang.data?.[0]?.community,
    ),
    releaseyear: compareYears(
      selectedLang.data?.[0]?.releaseyear,
      correctLang.data?.[0]?.releaseyear,
    ),
  };
  return new Response(
    JSON.stringify(data),
